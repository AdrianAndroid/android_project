...
public class LinearLayoutManager extends RecyclerView.LayoutManager implements
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (!mAnchorInfo.mValid || mPendingScrollPosition != RecyclerView.NO_POSITION || mPendingSavedState != null) {
            ...
            (mOrientationHelper.getDecoratedStart(focused) >= mOrientationHelper.getEndAfterPadding() || mOrientationHelper.getDecoratedEnd(focused) <= mOrientationHelper.getStartAfterPadding())
        ) {
     ...
     * 方法在确定锚点位置时调用。扩展类可以相应地设置，甚至在必要时更新锚点信息。
    ...
    int fill(RecyclerView.Recycler recycler, LayoutState layoutState, RecyclerView.State state, boolean stopOnFocusable) {
        ...
        while ((layoutState.mInfinite || remainingSpace > 0) && layoutState.hasMore(state)) {
            ...
            layoutChunk(recycler, state, layoutState, layoutChunkResult); // layoutChunk
            if (RecyclerView.VERBOSE_TRACING) {
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state, LayoutState layoutState, LayoutChunkResult result) {
        View view = layoutState.next(recycler); // 获得View
        if (view == null) {
        ...
        measureChildWithMargins(view, 0, 0); // 测量child的大小
        ...
        layoutDecoratedWithMargins(view, left, top, right, bottom); // layout
        if (DEBUG) {
    ...
    View findOneVisibleChild(int fromIndex, int toIndex, boolean completelyVisible, boolean acceptPartiallyVisible) {
        ...
        return (mOrientation == HORIZONTAL)
            ? mHorizontalBoundCheck.findOneViewWithinBoundFlags(fromIndex, toIndex, preferredBoundsFlag, acceptableBoundsFlag)
            : mVerticalBoundCheck.findOneViewWithinBoundFlags(fromIndex, toIndex, preferredBoundsFlag, acceptableBoundsFlag);
    }
    ...
    public void prepareForDrop(@NonNull View view, @NonNull View target, int x, int y) {
        ...
        if (mShouldReverseLayout) {
            ...
            if (dropDirection == LayoutState.ITEM_DIRECTION_HEAD) {
                ...
                scrollToPositionWithOffset(targetPos,
                        mOrientationHelper.getDecoratedEnd(target) - mOrientationHelper.getDecoratedMeasurement(view));
            }
        }
    }
