...
public class TaskDispatcher {
    ...
    private final AtomicInteger mNeedWaitCount = new AtomicInteger();//保存需要Wait的Task的数量
    private final List<Task> mNeedWaitTasks = new ArrayList<>();//调用了await的时候还没结束的且需要等待的Task
    private final List<Class<? extends Task>> mFinishedTasks = new ArrayList<>(100);//已经结束了的Task
    private final HashMap<Class<? extends Task>, ArrayList<Task>> mDependedHashMap = new HashMap<>();
    private final AtomicInteger mAnalyseCount = new AtomicInteger();//启动器分析的次数，统计下分析的耗时；
