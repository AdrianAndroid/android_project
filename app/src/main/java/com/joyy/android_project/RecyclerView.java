...
public class RecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild2, NestedScrollingChild3 {
    ...
    /**
     * Constant for use with {@link #setScrollingTouchSlop(int)}. Indicates that the RecyclerView should use the standard touch slop for smooth, continuous scrolling.
     * 用于{@link #setScrollingTouchSlop(int)}的常量。指示RecyclerView应该使用标准的触摸slop来实现平滑、连续的滚动。
     */
    public static final int TOUCH_SLOP_DEFAULT = 0;

    /**
     * Constant for use with {@link #setScrollingTouchSlop(int)}. Indicates that the RecyclerView should use the standard touch slop for scrolling widgets that snap to a page or other coarse-grained barrier.
     * 用于{@link #setScrollingTouchSlop(int)}的常量。指示RecyclerView应该使用标准的触摸slop来滚动小部件，这些小部件捕捉到一个页面或其他粗粒度的屏障。
     */
     ...
     * 处理LayoutManager子节点和RecyclerView子节点之间的抽象
     */
    ...
    /**
     * Prior to L, there is no way to query this variable which is why we override the setter and track it here.
     * 在L之前，没有办法查询这个变量，这就是为什么我们覆盖setter并在这里跟踪它。
     */
    boolean mClipToPadding;

    /**
     * Note: this Runnable is only ever posted if:     注意:这个Runnable只会在以下情况下发布:
     * 1) We've been through first layout              1) 我们已经完成了第一个布局
     * 2) We know we have a fixed size (mHasFixedSize) 2) 我们知道我们有一个固定的大小(mHasFixedSize)
     * 3) We're attached                               3) 我们连接
     */
    ...
    /**
     * True if a call to requestLayout was intercepted and prevented from executing like normal and we plan on continuing with normal execution later.
     * 如果对requestLayout的调用被拦截并阻止正常执行，并且我们计划稍后继续正常执行，则为True。
     */
    ...
    /**
     * True after an event occurs that signals that the entire data set has changed. In that case, we cannot run any animations since we don't know what happened until layout.
     * ，表示整个数据集已更改的事件发生后为。在这种情况下，我们不能运行任何动画，因为我们不知道发生了什么，直到布局。
     * <p>
     ...
     * 在下一个布局之前，附加的项是无效的，此时布局将根据需要动画/替换项，从(有效地)新适配器从头构建内容。
     * <p>
     * Cached items must be discarded when setting this to true, so that the cache may be freely used by prefetching until the next layout occurs.
     * 当设置为true时，缓存项必须被丢弃，以便缓存可以被预取使用，直到下一次布局发生。
     *
    ...
    /**
     * True after the data set has completely changed and {@link LayoutManager#onItemsChanged(RecyclerView)} should be called during the subsequent measure/layout.
     * 在数据集完全改变后，{@link LayoutManager#onItemsChanged(RecyclerView)}应该在后续的度量/布局中被调用。
     * @see #processDataSetCompletelyChanged(boolean)
     */
    boolean mDispatchItemsChangedEvent = false;

    /**
     * This variable is incremented during a dispatchLayout and/or scroll. Some methods should not be called during these periods (e.g. adapter data change). Doing so will create hard to find bugs so we better check it and throw an exception.
     * 该变量在dispatchLayout和/或滚动过程中递增。有些方法不应该在这些期间调用(例如，适配器数据更改)。这样做将很难发现错误，所以我们最好检查它并抛出异常。
     ...
     * 类似于mLayoutOrScrollCounter，但记录警告而不是抛出异常(为了API兼容性)。
     * <p>
     * It is a bad practice for a developer to update the data in a scroll callback since it is potentially called during a layout.
     * 对于开发人员来说，在滚动回调中更新数据是一种糟糕的做法，因为它可能在布局期间被调用。
     */
    ...
    private VelocityTracker mVelocityTracker; // 滑动追踪
    ...
    public RecyclerView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
         ...
         *  更改此视图是否为其窗口中的可滚动容器集之一。这将用于确定当软输入区域打开时窗口是否可以调整大小或必须平移——可滚动容器允许窗口使用调整大小模式，因为容器将适当收缩
         ...
         * 如果这个视图自己不绘图，设置这个标志以允许进一步优化。默认情况下，这个标志不会在View上设置，但是可以在一些View子类上设置，比如ViewGroup。
         * Typically, if you override {@link #onDraw(android.graphics.Canvas)} you should clear this flag.
         * 通常，如果你覆盖了{@link #onDraw(android.graphics.Canvas)}，你应该清除这个标记。
        ...
        if (mEnableFastScroller) {
            StateListDrawable verticalThumbDrawable =
                    (StateListDrawable) a.getDrawable(R.styleable.RecyclerView_fastScrollVerticalThumbDrawable);
            ...
            StateListDrawable horizontalThumbDrawable =
                    (StateListDrawable) a.getDrawable(R.styleable.RecyclerView_fastScrollHorizontalThumbDrawable);
            Drawable horizontalTrackDrawable =
                    a.getDrawable(R.styleable.RecyclerView_fastScrollHorizontalTrackDrawable);
            initFastScroller(verticalThumbDrawable, verticalTrackDrawable, horizontalThumbDrawable,
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            ...
            if (Build.VERSION.SDK_INT >= 29) {
                saveAttributeDataForStyleable(context, NESTED_SCROLLING_ATTRS, attrs, a, defStyleAttr, 0);
            }
    ...
    private void initChildrenHelper() {
        mChildHelper = new ChildHelper(new ChildHelper.Callback() {
            ...
            // 获取当前的View ViewGroup#getChildAt(offset)
            @Override
     ...
     * {@link ViewHolder} and does not clear the RecycledViewPool. // 不清空原来的缓存
     * <p>
     ...
     * or free those resources.
     * </p>
     * onChildViewAttachedToWindow / onChildViewDetachedFromWindow
    ...
    public void setLayoutManager(@Nullable LayoutManager layout) {
        ...
        if (layout != null) {
            ...
            mLayout.setRecyclerView(this); // 重新设置RecyclerView
            if (mIsAttached) {
                mLayout.dispatchAttachedToWindow(this);
            }
        }
        mRecycler.updateViewCacheSize();
        requestLayout(); // 重新从onMeasure->onLayout->onDraw()
    }
    ...
    void setScrollState(int state) {
        ...
        if (state != SCROLL_STATE_SETTLING) {
            stopScrollersInternal(); // 停止上次的滑动
        }
        dispatchOnScrollStateChanged(state); // 分发状态
    }
    ...
    boolean scrollByInternal(int x, int y, MotionEvent ev) {
        ...
        if (mAdapter != null) {
            ...
            scrollStep(x, y, mReusableIntPair); // 这里
    ...
    /**
     * Checks if the new focus candidate is a good enough candidate such that RecyclerView will assign it as the next focus View instead of letting view hierarchy decide.
     * 检查新的焦点候选是否足够好，以便RecyclerView将它分配为下一个焦点视图，而不是让视图层次决定。
     * A good candidate means a View that is aligned in the focus direction wrt the focused View and is not the RecyclerView itself.
     * 一个好的候选人意味着一个视图，是在焦点方向wrt聚焦的视图，而不是RecyclerView本身。
     * When this method returns false, RecyclerView will let the parent make the decision so the same View may still get the focus as a result of that search.
     * 当这个方法返回false时，RecyclerView将让父视图做决定，这样同样的视图仍然可以得到搜索结果的焦点。
     */
     ...
     * @param focused The descendant view that actually has the focus if child is focusable, null otherwise.
     */
    ...
    private boolean dispatchToOnItemTouchListeners(MotionEvent e) {
        ...
        //
        // 1. A single MotionEvent should not be passed to either OnInterceptTouchEvent or OnTouchEvent twice.
        // 2. ACTION_DOWN MotionEvents may be passed to both onInterceptTouchEvent and onTouchEvent.
        // 3. All other MotionEvents should be passed to either onInterceptTouchEvent or onTouchEvent, not both.
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        final boolean canScrollHorizontally = mLayout.canScrollHorizontally(); // mOrientation == HORIZONTAL
        final boolean canScrollVertically = mLayout.canScrollVertically(); // mOrientation == VERTICAL

        if (mVelocityTracker == null) { // https://www.jianshu.com/p/f97106d1f1f0 滑动速度跟踪器VelocityTracker
            mVelocityTracker = VelocityTracker.obtain();
        }
        boolean eventAddedToVelocityTracker = false;

        final int action = e.getActionMasked(); // action
        final int actionIndex = e.getActionIndex(); // 第几个手指头action

        if (action == MotionEvent.ACTION_DOWN) { // 记录
            mNestedOffsets[0] = mNestedOffsets[1] = 0;
        }
        final MotionEvent vtev = MotionEvent.obtain(e); // Create a new MotionEvent, copying from an existing one.
        vtev.offsetLocation(mNestedOffsets[0], mNestedOffsets[1]); // Adjust this event's location.

        switch (action) {
            case MotionEvent.ACTION_DOWN: {
                mScrollPointerId = e.getPointerId(0); // 第0个按下的id
                mInitialTouchX = mLastTouchX = (int) (e.getX() + 0.5f); // 坐标x
                mInitialTouchY = mLastTouchY = (int) (e.getY() + 0.5f); // 坐标y

                int nestedScrollAxis = ViewCompat.SCROLL_AXIS_NONE; // 是否是nested
                if (canScrollHorizontally) { // 横向
                    nestedScrollAxis |= ViewCompat.SCROLL_AXIS_HORIZONTAL;
                }
                if (canScrollVertically) { // 纵向
                    nestedScrollAxis |= ViewCompat.SCROLL_AXIS_VERTICAL;
                }
                startNestedScroll(nestedScrollAxis, TYPE_TOUCH); // 开始startNestedScroll
            }
            break;

            case MotionEvent.ACTION_POINTER_DOWN: { // 一个非主指针按下
                mScrollPointerId = e.getPointerId(actionIndex);
                mInitialTouchX = mLastTouchX = (int) (e.getX(actionIndex) + 0.5f);
                mInitialTouchY = mLastTouchY = (int) (e.getY(actionIndex) + 0.5f);
            }
            ...
            case MotionEvent.ACTION_MOVE: { // 移动
                final int index = e.findPointerIndex(mScrollPointerId); // 第几个索引
                if (index < 0) {
                    Log.e(TAG, "Error processing scroll; pointer index for id " + mScrollPointerId + " not found. Did any MotionEvents get skipped?");
                    return false;
                }

                final int x = (int) (e.getX(index) + 0.5f); // x
                final int y = (int) (e.getY(index) + 0.5f); // y
                int dx = mLastTouchX - x; // x偏移的距离
                int dy = mLastTouchY - y; // y偏移的距离

                if (mScrollState != SCROLL_STATE_DRAGGING) {
                    boolean startScroll = false; // 开始scorll
                    if (canScrollHorizontally) { // 横向
                        if (dx > 0) {
                    ...
                    if (startScroll) {
                        setScrollState(SCROLL_STATE_DRAGGING); // 设置拖动的状态
                    }
                }
            ...
            case MotionEvent.ACTION_UP: {
                mVelocityTracker.addMovement(vtev); // 添加
                eventAddedToVelocityTracker = true;
                mVelocityTracker.computeCurrentVelocity(1000, mMaxFlingVelocity);
                final float xvel = canScrollHorizontally ? -mVelocityTracker.getXVelocity(mScrollPointerId) : 0; // 横向的速度
                final float yvel = canScrollVertically ? -mVelocityTracker.getYVelocity(mScrollPointerId) : 0; // 纵向的速度
                if (!((xvel != 0 || yvel != 0) && fling((int) xvel, (int) yvel))) {
                    setScrollState(SCROLL_STATE_IDLE); // 设置STATE_IDLE状态
                }
                resetScroll();
            }
            break;
            ...
            case MotionEvent.ACTION_CANCEL: {
                cancelScroll(); // 取消，比如：退出到桌面
            }
            break;
        }

        if (!eventAddedToVelocityTracker) {
            mVelocityTracker.addMovement(vtev); // 滑动速度跟踪器
        }
        vtev.recycle();
    ...
    private void resetScroll() {
        ...
        stopNestedScroll(TYPE_TOUCH); // 停止NESTED
        releaseGlows();
    }
    ...
    private void onPointerUp(MotionEvent e) { // update
        final int actionIndex = e.getActionIndex();
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        ...
        if (mLayout.isAutoMeasureEnabled()) {
            ...
            final int widthMode = MeasureSpec.getMode(widthSpec); // UNSPECIFIED AT_MOST EXACTLY
            final int heightMode = MeasureSpec.getMode(heightSpec); // UNSPECIFIED AT_MOST EXACTLY
             ...
             */
            mLayout.onMeasure(mRecycler, mState, widthSpec, heightSpec); // 设置布局
    ...
    void dispatchLayout() {
        ...
        if (mState.mLayoutStep == State.STEP_START) {
            dispatchLayoutStep1(); // 记录RecyclerView刷新前ItemView的各种信息，如TOP，LEFT，BOTTOM，RIGHT，用于动画的相关计算
            mLayout.setExactMeasureSpecsFrom(this);
            // 做出呈现在手机的视图效果，
            // 具体过程：调用实现子类的onLayoutChildren()函数，计算所有子View的边界值，根据计算出的Child的边界计算出RecyclerView的所需width和height，并设置。
            dispatchLayoutStep2();
    ...
    private void saveFocusInfo() {
        ...
        if (focusedVh == null) {
            ...
            mState.mFocusedItemPosition = mDataSetHasChangedAfterLayout ? NO_POSITION
                    : (focusedVh.isRemoved() ? focusedVh.mOldPosition : focusedVh.getAdapterPosition());
     ...
     * - decide which animation should run     决定动画怎么运行
     * - save information about current views  把当前views的信息保存下来
     * - If necessary, run predictive layout and save its information 如果有必要，提前预测layout和保存信息
     */
    private void dispatchLayoutStep1() {
        mState.assertLayoutStep(State.STEP_START); // 判断否是是STE_START(开始)状态
        fillRemainingScrollValues(mState);
        mState.mIsMeasuring = false;
        startInterceptRequestLayout(); // 开始layout
        mViewInfoStore.clear(); // 清空原先的状态（动画）
        onEnterLayoutOrScroll();
        processAdapterUpdatesAndSetAnimationFlags();
        saveFocusInfo();
        ...
        if (mState.mRunSimpleAnimations) {
            ...
            for (int i = 0; i < count; ++i) {
                ...
                final ItemAnimator.ItemHolderInfo animationInfo = mItemAnimator.recordPreLayoutInformation(mState, holder, ItemAnimator.buildAdapterChangeFlagsForAnimations(holder), holder.getUnmodifiedPayloads());
        ...
        if (mState.mRunPredictiveAnimations) {
            ...
            for (int i = 0; i < mChildHelper.getChildCount(); ++i) {
                ...
                if (!mViewInfoStore.isInPreLayout(viewHolder)) {
                    ...
                    final ItemAnimator.ItemHolderInfo animationInfo = mItemAnimator.recordPreLayoutInformation(mState,
                            viewHolder, flags, viewHolder.getUnmodifiedPayloads());
                    if (wasHidden) {
        ...
        onExitLayoutOrScroll();
        stopInterceptRequestLayout(false); // 结束layout
    ...
    private void dispatchLayoutStep3() {
        ...
        if (mState.mRunSimpleAnimations) {
            ...
            for (int i = mChildHelper.getChildCount() - 1; i >= 0; i--) {
                ...
                long key = getChangedHolderKey(holder);
                final ItemAnimator.ItemHolderInfo animationInfo =
                        mItemAnimator.recordPostLayoutInformation(mState, holder);
                ...
                if (oldChangeViewHolder != null && !oldChangeViewHolder.shouldIgnore()) {
                    ...
                    if (oldDisappearing && oldChangeViewHolder == holder) {
                        ...
                        final ItemAnimator.ItemHolderInfo preInfo =
                                mViewInfoStore.popFromPreLayout(oldChangeViewHolder);
                        ...
                        if (preInfo == null) {
                            ...
                            animateChange(oldChangeViewHolder, holder, preInfo, postInfo, oldDisappearing,
                                    newDisappearing);
                        }
                    }
                } else {
    ...
    private void handleMissingPreInfoForChangeError(long key, ViewHolder holder, ViewHolder oldChangeViewHolder) {
            ...
            if (otherKey == key) {
                if (mAdapter != null && mAdapter.hasStableIds()) {
                    throw new IllegalStateException("Two different ViewHolders have the same stable"
                            + " ID. Stable IDs in your adapter MUST BE unique and SHOULD NOT" + " change.\n ViewHolder 1:"
        ...
        Log.e(TAG,
                "Problem while matching changed view holders with the new"
    ...
    void recordAnimationInfoIfBouncedHiddenView(ViewHolder viewHolder, ItemAnimator.ItemHolderInfo animationInfo) {
        ...
        if (mState.mTrackOldChangeHolders && viewHolder.isUpdated() && !viewHolder.isRemoved()
                && !viewHolder.shouldIgnore()) {
    ...
    void animateAppearance(@NonNull ViewHolder itemHolder, @Nullable ItemAnimator.ItemHolderInfo preLayoutInfo,
                           @NonNull ItemAnimator.ItemHolderInfo postLayoutInfo) {
    ...
    void animateDisappearance(@NonNull ViewHolder holder, @NonNull ItemAnimator.ItemHolderInfo preLayoutInfo,
                              @Nullable ItemAnimator.ItemHolderInfo postLayoutInfo) {
    ...
    /**
     * 总结:
     * 1. RecyclerView并没有对内部的View进行布局。而是交给LayoutManager去做具体的布局操作，
     * 因此才会有LinearLayoutManaager，GridLayoutManager，StaggeredGridLayoutManager这样灵活的布局。
     * 而且我们可以通过实现onLayoutChildren,自定义LayoutManager。
     * 2. RecyclerView的布局过程中包含很多动画相关的处理
     * 3. RecyclerView的数据改变的动画是在布局过程中的第三步中统一触发的。并不是一调用notify之后立即触发.
     */
    @Override
    ...
    void markItemDecorInsetsDirty() {
        final int childCount = mChildHelper.getUnfilteredChildCount(); // 多少个childView
        for (int i = 0; i < childCount; i++) {
            final View child = mChildHelper.getUnfilteredChildAt(i); // getChildAt(i)
    ...
    // 除了绘制自己外，还多调用了mItemDecorations的onDraw()方法。我们知道onDraw调用之前。
    // 会先调用draw()方法，Recyclerview重写了draw方法：也就是先用draw()绘制item，然后再调用
    // mItemDecorations的onDraw()来加一层图片，还要调用getItemOffsets()，让item的view发生一定的偏移
    // 让在ItemView下的图片可以显示出来
    @Override
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (DEBUG && holder.mPosition == -1 && !holder.isRemoved()) {
                throw new IllegalStateException(
                        "view holder cannot have position -1 unless it" + " is removed" + exceptionLabel());
            }
            if (!holder.shouldIgnore()) {
    ...
    void offsetPositionRecordsForInsert(int positionStart, int itemCount) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (holder != null && !holder.shouldIgnore() && holder.mPosition >= positionStart) {
                if (DEBUG) {
                    Log.d(TAG, "offsetPositionRecordsForInsert attached child " + i + " holder " + holder
                            + " now at position " + (holder.mPosition + itemCount));
                }
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (holder != null && !holder.shouldIgnore()) {
                if (holder.mPosition >= positionEnd) {
                    if (DEBUG) {
                        Log.d(TAG, "offsetPositionRecordsForRemove attached child " + i + " holder " + holder
                                + " now at position " + (holder.mPosition - itemCount));
                    }
    ...
    boolean canReuseUpdatedViewHolder(ViewHolder viewHolder) {
        return mItemAnimator == null
                || mItemAnimator.canReuseUpdatedViewHolder(viewHolder, viewHolder.getUnmodifiedPayloads());
    }
     ...
     * data change event.
     * 1。所有的FLAG标记为可以更新
     * 2。区域DecorInset标记为dirty
     * 3。缓存的view标记可以更新
     */
    void markKnownViewsInvalid() {
        final int childCount = mChildHelper.getUnfilteredChildCount(); // 多少个childview
        for (int i = 0; i < childCount; i++) {
            final ViewHolder holder = getChildViewHolderInt(mChildHelper.getUnfilteredChildAt(i)); // 找到ViewHolder
            if (holder != null && !holder.shouldIgnore()) {
                holder.addFlags(ViewHolder.FLAG_UPDATE | ViewHolder.FLAG_INVALID); // 添加更新标志位
            }
        }
        markItemDecorInsetsDirty();
    ...
    class ViewFlinger implements Runnable {
        ...
        public void run() {
            ...
            if (scroller.computeScrollOffset()) {
                ...
                final boolean doneScrolling = scroller.isFinished()
                        || ((scrollerFinishedX || unconsumedX != 0) && (scrollerFinishedY || unconsumedY != 0));
        ...
        public void fling(int velocityX, int velocityY) {
            ...
            mOverScroller.fling(0, 0, velocityX, velocityY, Integer.MIN_VALUE, Integer.MAX_VALUE, Integer.MIN_VALUE,
                    Integer.MAX_VALUE);
            postOnAnimation();
        }
     ...
     * EdgeEffectFactory可以让你为RecyclerViews定制滚屏边缘效果。
    ...
    public static class EdgeEffectFactory {
         ...
         */
        protected @NonNull
        EdgeEffect createEdgeEffect(@NonNull RecyclerView view, @EdgeDirection int direction) {
    ...
    public static class RecycledViewPool {
        ...
        static class ScrapData {
            final ArrayList<ViewHolder> mScrapHeap = new ArrayList<>(); // 每个viewType都是5个

            int mMaxScrap = DEFAULT_MAX_SCRAP; // max -> 5

            long mCreateRunningAverageNs = 0; // 创建的平均事件

            long mBindRunningAverageNs = 0; // 绑定的平均事件
        }
        ...
        public void putRecycledView(ViewHolder scrap) {
            final int viewType = scrap.getItemViewType(); // 获取ViewType
            final ArrayList<ViewHolder> scrapHeap = getScrapDataForType(viewType).mScrapHeap;
            if (mScrap.get(viewType).mMaxScrap <= scrapHeap.size()) { // 存够最大5个，就不存了
                return;
            }
            if (DEBUG && scrapHeap.contains(scrap)) {
                throw new IllegalArgumentException("this scrap item already exists");
            }
            scrap.resetInternal(); // 重置ViewHolder所有的数据
            scrapHeap.add(scrap); // 存进去
        }
    ...
    public final class Recycler {
        ...
        void updateViewCacheSize() {
            ...
            mViewCacheMax = mRequestedCacheMax + extraCache; // mRequestedCacheMax==5
        ...
        boolean validateViewHolderForOffsetPosition(ViewHolder holder) {
            ...
            if (holder.isRemoved()) {
                if (DEBUG && !mState.isPreLayout()) {
                    throw new IllegalStateException(
                            "should not receive a removed view unless it" + " is pre layout" + exceptionLabel());
                }
                return mState.isPreLayout();
            }
            if (holder.mPosition < 0 || holder.mPosition >= mAdapter.getItemCount()) {
                throw new IndexOutOfBoundsException(
                        "Inconsistency detected. Invalid view holder " + "adapter position" + holder + exceptionLabel());
            }
        ...
        public void bindViewToPosition(@NonNull View view, int position) {
            ...
            if (holder == null) {
                throw new IllegalArgumentException("The view does not have a ViewHolder. You cannot"
                        + " pass arbitrary views to this method, they should be created by the " + "Adapter"
                        + exceptionLabel());
            }
            final int offsetPosition = mAdapterHelper.findPositionOffset(position);
            if (offsetPosition < 0 || offsetPosition >= mAdapter.getItemCount()) {
                throw new IndexOutOfBoundsException("Inconsistency detected. Invalid item " + "position " + position
        ...
        public int convertPreLayoutPositionToPostLayout(int position) {
            if (position < 0 || position >= mState.getItemCount()) {
                throw new IndexOutOfBoundsException("invalid position " + position + ". State " + "item count is "
        ...
        public void recycleView(@NonNull View view) {
            ...
            ViewHolder holder = getChildViewHolderInt(view); // 先拿出ViewHolder
            if (holder.isTmpDetached()) { // 临时detached
        ...
        void recycleCachedViewAt(int cachedViewIndex) {
            ...
            ViewHolder viewHolder = mCachedViews.get(cachedViewIndex); // 取出当前cache
            if (DEBUG) {
                Log.d(TAG, "CachedViewHolder to be recycled: " + viewHolder);
            }
            addViewHolderToRecycledViewPool(viewHolder, true); // 添加到缓存池中
        ...
        void recycleViewHolderInternal(ViewHolder holder) {
            ...
            if (forceRecycle || holder.isRecyclable()) {
                // ViewHolder改变不会进来， 先判断mCachedViews的大小
        ...
        void addViewHolderToRecycledViewPool(@NonNull ViewHolder holder, boolean dispatchRecycled) {
            ...
            holder.mOwnerRecyclerView = null; // 将ViewHolder拥有的RecyclVerView清空
            getRecycledViewPool().putRecycledView(holder); // 加入到缓存池中
        }
        ...
        void scrapView(View view) {
            ...
            if (holder.hasAnyOfTheFlags(ViewHolder.FLAG_REMOVED | ViewHolder.FLAG_INVALID) || !holder.isUpdated() || canReuseUpdatedViewHolder(holder)) {
                log("scrapView true == holder.hasAnyOfTheFlags(ViewHolder.FLAG_REMOVED | ViewHolder.FLAG_INVALID) || !holder.isUpdated() || canReuseUpdatedViewHolder(holder)");
                ...
                log("scrapView false == holder.hasAnyOfTheFlags(ViewHolder.FLAG_REMOVED | ViewHolder.FLAG_INVALID) || !holder.isUpdated() || canReuseUpdatedViewHolder(holder)");
         ...
         * 这个视图将不再适合重用，直到被重新废弃或者直到它被显式地移除并回收。
         * </p>
         */
        ...
        ViewHolder getScrapOrHiddenOrCachedHolderForPosition(int position, boolean dryRun) {
            ...
            log("getScrapOrHiddenOrCachedHolderForPosition scrapCount = " + scrapCount);
            ...
            if (!dryRun) {
                ...
                if (view != null) {
                    ...
                    if (layoutIndex == RecyclerView.NO_POSITION) {
                        throw new IllegalStateException(
                                "layout index should not be -1 after " + "unhiding a view:" + vh + exceptionLabel());
                    }
            ...
            for (int i = 0; i < cacheSize; i++) {
                final ViewHolder holder = mCachedViews.get(i); // 最大是两个
                ...
                if (holder.getItemId() == id && !holder.wasReturnedFromScrap()) {
                    if (type == holder.getItemViewType()) {
                        ...
                        if (holder.isRemoved()) {
                            ...
                            // between.
                            if (!mState.isPreLayout()) {
                                holder.setFlags(ViewHolder.FLAG_UPDATE,
        ...
        void offsetPositionRecordsForInsert(int insertedAt, int count) {
            ...
            for (int i = 0; i < cachedCount; i++) {
                ...
                if (holder != null && holder.mPosition >= insertedAt) {
                    if (DEBUG) {
                        Log.d(TAG, "offsetPositionRecordsForInsert cached " + i + " holder " + holder
                                + " now at position " + (holder.mPosition + count));
                    }
            ...
            for (int i = cachedCount - 1; i >= 0; i--) {
                ...
                if (holder != null) {
                    if (holder.mPosition >= removedEnd) {
                        if (DEBUG) {
                            Log.d(TAG, "offsetPositionRecordsForRemove cached " + i + " holder " + holder
                                    + " now at position " + (holder.mPosition - count));
                        }
    ...
    public abstract static class Adapter<VH extends ViewHolder> {
         ...
         * </p>
         * <p>
         * <code>RecyclerView</code> will attempt to synthesize visible structural change events
    ...
    void dispatchChildDetached(View child) {
        ...
        if (mAdapter != null && viewHolder != null) {
            mAdapter.onViewDetachedFromWindow(viewHolder); // 回调
        }
        if (mOnChildAttachStateListeners != null) {
    ...
    public abstract static class LayoutManager {
         ...
         * <p>
         * LayoutManagers will often want to use a custom <code>LayoutParams</code> type
         * to store extra information specific to the layout. Client code should subclass
         * {@link RecyclerView.LayoutParams} for this purpose.
         * </p>
         * <p>
         * <em>Important:</em> if you use your own custom <code>LayoutParams</code> type
         ...
         * returns <code>true</code> for non-null params.
         * </p>
         *
        ...
        public void moveView(int fromIndex, int toIndex) {
            ...
            if (view == null) {
                throw new IllegalArgumentException(
                        "Cannot move a child from non-existing index:" + fromIndex + mRecyclerView.toString());
            }
        ...
        public void removeAndRecycleViewAt(int index, @NonNull Recycler recycler) {
            ...
            removeViewAt(index);
            recycler.recycleView(view); // 回收起来
        }
        ...
        public void ignoreView(@NonNull View view) {
            if (view.getParent() != mRecyclerView || mRecyclerView.indexOfChild(view) == -1) {
                ...
                // cause loss of state.
                throw new IllegalArgumentException(
                        "View should be fully attached to be ignored" + mRecyclerView.exceptionLabel());
            }
            final ViewHolder vh = getChildViewHolderInt(view);
        ...
        boolean shouldReMeasureChild(View child, int widthSpec, int heightSpec, LayoutParams lp) {
            return !mMeasurementCacheEnabled || !isMeasurementUpToDate(child.getMeasuredWidth(), widthSpec, lp.width)
        ...
        boolean shouldMeasureChild(View child, int widthSpec, int heightSpec, LayoutParams lp) {
            return child.isLayoutRequested() || !mMeasurementCacheEnabled
        ...
        public void measureChildWithMargins(@NonNull View child, int widthUsed, int heightUsed) {
            ...
            final int widthSpec = getChildMeasureSpec(getWidth(), getWidthMode(),
                getPaddingLeft() + getPaddingRight() + lp.leftMargin + lp.rightMargin + widthUsed, lp.width, canScrollHorizontally());
            final int heightSpec = getChildMeasureSpec(getHeight(), getHeightMode(),
                getPaddingTop() + getPaddingBottom() + lp.topMargin + lp.bottomMargin + heightUsed, lp.height, canScrollVertically());
            if (shouldMeasureChild(child, widthSpec, heightSpec, lp)) {
                child.measure(widthSpec, heightSpec);
            }
        }
         ...
         * @param childDimension Desired size of the child view, or MATCH_PARENT/WRAP_CONTENT. Generally obtained from the child view's LayoutParams
         * @param canScroll      true if the parent RecyclerView can scroll in this dimension
         * @return a MeasureSpec value for the child view
         */
        ...
        public void layoutDecoratedWithMargins(@NonNull View child, int left, int top, int right, int bottom) {
            ...
            child.layout(left + insets.left + lp.leftMargin,
        ...
        public void getTransformedBoundingBox(@NonNull View child, boolean includeDecorInsets, @NonNull Rect out) {
            ...
            if (mRecyclerView != null) {
                ...
                if (childMatrix != null && !childMatrix.isIdentity()) {
                    ...
                    out.set((int) Math.floor(tempRectF.left), (int) Math.floor(tempRectF.top),
                            (int) Math.ceil(tempRectF.right), (int) Math.ceil(tempRectF.bottom));
                }
            }
         ...
         */
        @Nullable
        public View onFocusSearchFailed(@NonNull View focused, int direction, @NonNull Recycler recycler,
                                        @NonNull State state) {
            return null;
        }
        ...
        public boolean isViewPartiallyVisible(@NonNull View child, boolean completelyVisible,
            ...
            boolean isViewFullyVisible = mHorizontalBoundCheck.isViewWithinBoundFlags(child, boundsFlag)
        ...
        private boolean isFocusedChildVisibleAfterScrolling(RecyclerView parent, int dx, int dy) {
            ...
            return bounds.left - dx < parentRight && bounds.right - dx > parentLeft && bounds.top - dy < parentBottom
         ...
         */
        public boolean onRequestChildFocus(@NonNull RecyclerView parent, @NonNull State state, @NonNull View child,
         ...
         * <p>
         * The LayoutManager implementation should return <code>true</code> if the default
         * behavior of {@link ViewGroup#addFocusables(java.util.ArrayList, int)} should be
         * suppressed.
         * </p>
         * <p>
         * The default implementation returns <code>false</code> to trigger RecyclerView
         ...
         */
        public void onInitializeAccessibilityNodeInfo(@NonNull Recycler recycler, @NonNull State state,
            ...
            final AccessibilityNodeInfoCompat.CollectionInfoCompat collectionInfo =
                    AccessibilityNodeInfoCompat.CollectionInfoCompat.obtain(getRowCountForAccessibility(recycler, state),
        ...
        public void onInitializeAccessibilityEvent(@NonNull Recycler recycler, @NonNull State state,
            ...
            event.setScrollable(mRecyclerView.canScrollVertically(1) || mRecyclerView.canScrollVertically(-1)
                    || mRecyclerView.canScrollHorizontally(-1) || mRecyclerView.canScrollHorizontally(1));
         ...
         * android.view.accessibility.AccessibilityNodeInfo)
         */
        public void onInitializeAccessibilityNodeInfoForItem(@NonNull Recycler recycler, @NonNull State state,
                                                             @NonNull View host, @NonNull AccessibilityNodeInfoCompat info) {
            int rowIndexGuess = canScrollVertically() ? getPosition(host) : 0;
            int columnIndexGuess = canScrollHorizontally() ? getPosition(host) : 0;
            final AccessibilityNodeInfoCompat.CollectionItemInfoCompat itemInfo =
                    AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(rowIndexGuess, 1, columnIndexGuess, 1,
                            false, false);
         ...
         */
        public boolean performAccessibilityAction(@NonNull Recycler recycler, @NonNull State state, int action,
         ...
         * @see View#performAccessibilityAction(int, android.os.Bundle)
         */
        public boolean performAccessibilityActionForItem(@NonNull Recycler recycler, @NonNull State state,
                                                         @NonNull View view, int action, @Nullable Bundle args) {
            return false;
        }
         ...
         */
        public static Properties getProperties(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr,
                                               int defStyleRes) {
    ...
    public abstract static class ViewHolder {
         ...
         * 这个ViewHolder已被绑定到一个位置;mPosition、mItemId和mItemViewType都是有效的。
         */
        static final int FLAG_BOUND = 1 << 0;

        /**
         * The data this ViewHolder's view reflects is stale and needs to be rebound by the adapter. mPosition and mItemId are consistent.
         * 这个ViewHolder的视图反映的数据已经过时，需要由适配器进行反弹。mPosition和mItemId一致。
         */
        static final int FLAG_UPDATE = 1 << 1;

        /**
         * This ViewHolder's data is invalid. The identity implied by mPosition and mItemId are not to be trusted and may no longer match the item view type. This ViewHolder must be fully rebound to different data.
         * 这个ViewHolder的数据无效。由mPosition和mItemId隐含的标识不受信任，可能不再与项目视图类型匹配。这个ViewHolder必须完全反弹到不同的数据。
         */
        static final int FLAG_INVALID = 1 << 2;

        /**
         * This ViewHolder points at data that represents an item previously removed from the data set. Its view may still be used for things like outgoing animations.
         * 此ViewHolder指向表示先前从数据集中删除的项的数据。它的视图仍然可以用于输出动画。
         */
        static final int FLAG_REMOVED = 1 << 3;

        /**
         * This ViewHolder should not be recycled. This flag is set via setIsRecyclable() and is intended to keep views around during animations.
         * 这个ViewHolder不应该被回收。这个标志是通过setIsRecyclable()设置的，目的是在动画期间保持周围的视图。
         */
        static final int FLAG_NOT_RECYCLABLE = 1 << 4;

        /**
         * This ViewHolder is returned from scrap which means we are expecting an addView call for this itemView. When returned from scrap, ViewHolder stays in the scrap list until the end of the layout pass and then recycled by RecyclerView if it is not added back to the RecyclerView.
         * 这个ViewHolder是从废料中返回的，这意味着我们期待对这个itemView调用addView。当从废料返回时，ViewHolder停留在废料列表中，直到布局结束，然后由RecyclerView回收，如果它没有添加回RecyclerView。
         * scrap 碎片/一丁点
         */
        static final int FLAG_RETURNED_FROM_SCRAP = 1 << 5;

        /**
         * This ViewHolder is fully managed by the LayoutManager. We do not scrap, recycle or remove it unless LayoutManager is replaced. It is still fully visible to the LayoutManager.
         * 这个ViewHolder完全由LayoutManager管理。我们不废弃，回收或删除它，除非LayoutManager被取代。它对LayoutManager仍然是完全可见的。
         */
        static final int FLAG_IGNORE = 1 << 7;

        /**
         * When the View is detached form the parent, we set this flag so that we can take correct action when we need to remove it or add it back.
         * 当View从父视图中分离出来时，我们设置这个标志，以便在需要删除或添加它时可以采取正确的操作。
         */
        static final int FLAG_TMP_DETACHED = 1 << 8;

        /**
         * Set when we can no longer determine the adapter position of this ViewHolder until it is rebound to a new position. It is different than FLAG_INVALID because FLAG_INVALID is set even when the type does not match. Also, FLAG_ADAPTER_POSITION_UNKNOWN is set as soon as adapter notification arrives vs FLAG_INVALID is set lazily before layout is re-calculated.
         * 当我们不能再确定这个ViewHolder的适配器位置时设置，直到它被反弹到一个新位置。它与FLAG_INVALID不同，因为即使类型不匹配，也会设置FLAG_INVALID。另外，FLAG_ADAPTER_POSITION_UNKNOWN在适配器通知到达时设置，而FLAG_INVALID在重新计算布局之前延迟设置。
         */
        static final int FLAG_ADAPTER_POSITION_UNKNOWN = 1 << 9;

        /**
         * Set when a addChangePayload(null) is called
         * 在调用addChangePayload(null)时设置
         */
        static final int FLAG_ADAPTER_FULLUPDATE = 1 << 10;

        /**
         * Used by ItemAnimator when a ViewHolder's position changes
         * 当ViewHolder的位置发生变化时由ItemAnimator使用
         */
        static final int FLAG_MOVED = 1 << 11;

        /**
         * Used by ItemAnimator when a ViewHolder appears in pre-layout
         * 当ViewHolder在预布局中出现时由ItemAnimator使用
         */
        ...
        /**
         * Used when a ViewHolder starts the layout pass as a hidden ViewHolder but is re-used from hidden list (as if it was scrap) without being recycled in between.
         * 当ViewHolder作为一个隐藏的ViewHolder开始布局时使用，但从隐藏列表中重用(就像它是废料)，而没有被回收。
         * <p>
         ...
         * <p>
         * This flag is used to represent "case b" where the ViewHolder is reused without being recycled (thus "bounced" from the hidden list). This state requires special handling because the ViewHolder must be added to pre layout maps for animations as if it was already there.
         * 此标志用于表示ViewHolder被重用而没有被回收的“情况b”(因此从隐藏列表中“反弹”)。这种状态需要特殊处理，因为ViewHolder必须添加到动画的预布局映射中，就像它已经存在一样。
         */
    ...
    int getAdapterPositionFor(ViewHolder viewHolder) {
        if (viewHolder.hasAnyOfTheFlags(
                ViewHolder.FLAG_INVALID | ViewHolder.FLAG_REMOVED | ViewHolder.FLAG_ADAPTER_POSITION_UNKNOWN)
                || !viewHolder.isBound()) {
    ...
    void initFastScroller(StateListDrawable verticalThumbDrawable, Drawable verticalTrackDrawable,
        ...
        new FastScroller(this, verticalThumbDrawable, verticalTrackDrawable, horizontalThumbDrawable,
                horizontalTrackDrawable, resources.getDimensionPixelSize(R.dimen.fastscroll_default_thickness),
    ...
    public abstract static class SmoothScroller {
        ...
        protected void normalize(@NonNull PointF scrollVector) {
            final float magnitude =
                    (float) Math.sqrt(scrollVector.x * scrollVector.x + scrollVector.y * scrollVector.y);
    ...
    public static class State {
        ...
        /**
         * True if the associated {@link RecyclerView} is in the pre-layout step where it is having its {@link LayoutManager} layout items where they will be at the beginning of a set of predictive item animations.
         * 如果关联的{@link RecyclerView}处于预布局步骤，其中它的{@link LayoutManager}布局项将位于一组预测项动画的开始。
         */
        ...
        public String toString() {
            return "State{" + "mTargetPosition=" + mTargetPosition + ", mData=" + mData + ", mItemCount=" + mItemCount
                    + ", mIsMeasuring=" + mIsMeasuring + ", mPreviousLayoutItemCount=" + mPreviousLayoutItemCount
                    + ", mDeletedInvisibleItemCountSincePreviousLayout=" + mDeletedInvisibleItemCountSincePreviousLayout
                    + ", mStructureChanged=" + mStructureChanged + ", mInPreLayout=" + mInPreLayout
                    + ", mRunSimpleAnimations=" + mRunSimpleAnimations + ", mRunPredictiveAnimations="
