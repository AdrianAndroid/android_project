...
public class ListView extends AbsListView {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.UNSPECIFIED) {
            ...
            widthSize |= (childState & View.MEASURED_STATE_MASK);
        }
    ...
    private void setupChild(View child, int position, int y, boolean flowDown, int childrenLeft,
        ...
        if (needToMeasure) {
            final int childWidthSpec = getChildMeasureSpec(mWidthMeasureSpec,
                    mListPadding.left + mListPadding.right, p.width);
            final int lpHeight = p.height;
            final int childHeightSpec;
            if (lpHeight > 0) {
            ...
            switch (keyCode) {
                case KeyEvent.KEYCODE_DPAD_UP:
                    if (event.hasNoModifiers()) {
                        ...
                        handled = resurrectSelectionIfNeeded() || fullScroll(View.FOCUS_UP);
                    }
                    break;
                ...
                case KeyEvent.KEYCODE_DPAD_DOWN:
                    if (event.hasNoModifiers()) {
                        ...
                        if (!handled) {
                            while (count-- > 0) {
                                if (arrowScroll(View.FOCUS_DOWN)) {
                                    handled = true;
                                } else {
                                    break;
                                }
                        ...
                        handled = resurrectSelectionIfNeeded() || fullScroll(View.FOCUS_DOWN);
                    }
                    break;
                ...
                case KeyEvent.KEYCODE_PAGE_UP:
                    if (event.hasNoModifiers()) {
                        handled = resurrectSelectionIfNeeded() || pageScroll(View.FOCUS_UP);
                    } else if (event.hasModifiers(KeyEvent.META_ALT_ON)) {
                        handled = resurrectSelectionIfNeeded() || fullScroll(View.FOCUS_UP);
                    }
                    break;
                ...
                case KeyEvent.KEYCODE_PAGE_DOWN:
                    if (event.hasNoModifiers()) {
                        handled = resurrectSelectionIfNeeded() || pageScroll(View.FOCUS_DOWN);
                    } else if (event.hasModifiers(KeyEvent.META_ALT_ON)) {
                        handled = resurrectSelectionIfNeeded() || fullScroll(View.FOCUS_DOWN);
                    }
                    break;
                ...
                case KeyEvent.KEYCODE_TAB:
                    ...
                    if (event.hasNoModifiers()) {
                        handled = resurrectSelectionIfNeeded() || arrowScroll(View.FOCUS_DOWN);
                    } else if (event.hasModifiers(KeyEvent.META_SHIFT_ON)) {
                        handled = resurrectSelectionIfNeeded() || arrowScroll(View.FOCUS_UP);
                    }
                    break;
            }
        }
