...
class Session extends IWindowSession.Stub implements IBinder.DeathRecipient {
    ...
    void windowAddedLocked(String packageName) {
        ...
        if (mSurfaceSession == null) {//为空才会继续执行，创建一个SurfaceSession对象并把自己加入到WMS的mSession列表中
            if (WindowManagerService.localLOGV) Slog.v(//保证一个Session不会重复加入到WMS的mSessions变量中
