...
class WindowState extends WindowContainer<WindowState> implements WindowManagerPolicy.WindowState {
    ...
    final Context mContext;//Session，是system_server的binder服务端
    final Session mSession;
    final IWindow mClient;//IWindow，是app端的ViewRootImpl.W服务的binder代理对象
    final int mAppOp;
     ...
     */
    final Rect mOverscanInsets = new Rect();//Window 的过扫描边衬区
     ...
     * application's coordinate space (without compatibility scale applied).
     */
    final Rect mStableInsets = new Rect();//Window 被系统Window 遮住的固定边衬区
     ...
     * to draw anyway.
     */
    final Rect mOutsets = new Rect();//不在Window 的Surface 区域但可以绘制的区域
    ...
    private final Rect mOverscanFrame = new Rect();//Overscan区域大小
    ...
    private final Rect mContentFrame = new Rect();//内容区域大小
    ...
    WindowState(WindowManagerService service, Session s, IWindow c, WindowToken token,
        ...
        if (mAttrs.type >= FIRST_SUB_WINDOW && mAttrs.type <= LAST_SUB_WINDOW) {
            ...
            mBaseLayer = mPolicy.getWindowLayerLw(parentWindow)
                    * TYPE_LAYER_MULTIPLIER + TYPE_LAYER_OFFSET;//计算mBaseLayer
            mSubLayer = mPolicy.getSubWindowLayerFromTypeLw(a.type);//计算mSubLayer
            mIsChildWindow = true;
            ...
            // The multiplier here is to reserve space for multiple
            // windows in the same type layer.
            mBaseLayer = mPolicy.getWindowLayerLw(this)//注：TYPE_LAYER_MULTIPLIER的值是10000，TYPE_LAYER_OFFSET的值是1000
