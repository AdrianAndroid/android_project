    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                setPressed(true);
                // 1。如果KeyButtonView被设置了mCode，则创建并发送给InputDispatcher一个ACTION_DOWN的按键事件
                if (mCode != 0) {
                ...
                // 倘若KeyButtonView被设置为支持长按事件，则发送一个名为mCheckLongPress的Runnable，并在延迟一段时间后执行它。
                // 这个Runnable会重新发送一个待用LONG_PRESS标记的ACITON_DOWN
            ...
            case MotionEvent.ACTION_CANCEL:
                setPressed(false);
                if (mCode != 0) {
                    // 2。当触摸事件被取消，则发送一个带有FLAG_CANCELED标记的ACTION_UP按键事件
