 ...
 * # 鉴于窗口布局和控件布局的一致性，WindowManager继承并实现了接口的ViewManager
 * # 使用者可以通过Context.getSystemService(Context.WINDOW_SERVICE)来获取一个WindowManager的实例。这个实例的最真实类型是WindowManagerImpl。WindowManagerImpl一旦被创建就确定了通过它所创建的窗口属于哪块屏幕、哪个父窗口
 * # WindowManagerImpl除了保存窗口所属的屏幕以及父窗口之外，没有任何实质性的工作。窗口的管理都交由WindowManagerGlobal的实例完成。
 * # WindowManagerGlobal在一个进程中只有一个实例
 * # WindowManagerGlobal在一个进程中只有一个实例
 * # WindowManagerGlobal在三个数组统一管理整个进程所有窗口的信息。这些信息包括控件、布局参数以及ViewRootImpl三个元素
 * # 床了管理窗口的上述三个元素之外，WindowManagerGlobal将窗口的创建、销毁与布局更新等任务交给ViewRootImpl完成。
 */
