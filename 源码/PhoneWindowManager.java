...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    public void layoutWindowLw(WindowState win, WindowManager.LayoutParams attrs,
        ...
        // 状态栏于导航栏不再被布局。事实上，这两个窗口是由beginLayoutLw函数在计算布局参数时完成布局的，
        // 这一点很容易理解，以为一些布局参数是依赖于状态栏和导航栏的尺寸
        ...
        //
        final Rect pf = mTmpParentFrame; // ParentFrame
        final Rect df = mTmpDisplayFrame; // DisplayFrame
        final Rect cf = mTmpContentFrame; // ContentFrame
        final Rect vf = mTmpVisibleFrame; // VisibleFrame
        ...
        // 下面这段代码于当前窗口的布局操作本身没有什么关系。如上一节所述，当完成输入法窗口的布局后
        // 需要更新Content区域于cur区域。这里就是完成这个工作的地方
     ...
     * mForceStatusBar 和 mForceStatusBarFromKeyguard
     * mHideLockScreen
     * mDismissKeyguard
     * @param win
     * @param attrs
    ...
    public void applyPostLayoutPolicyLw(WindowState win, WindowManager.LayoutParams attrs) {
        ...
        // 因此，可以得出一个结论：仅那些位于第一个可见的满屏窗口（含）之上的可见窗口才能影响系统窗口与锁屏界面的可见性
        ...
        if (mTopFullscreenOpaqueWindowState == null &&
            ...
            // 只有窗口拥有FLAG_FORCE_NOT_FULLSCREEN的flag，必然要求强制显示系统窗口
            ...
            // 如果有窗口的类型为TYPE_KEYGUARD，表示正在显示锁屏界面
            if (attrs.type == TYPE_KEYGUARD) {
            ...
            if (applyWindow
                ...
                // 将这个满屏的应用窗口记录到mTopFullscreenOpaqueWindowState中。之后对此函数的调用将不再做任何事情
                ...
                if ((attrs.flags & FLAG_DISMISS_KEYGUARD) != 0 && mDismissKeyguard == DISMISS_KEYGUARD_NONE) {
                    ...
                    // 如果此窗口已经解锁了锁屏界面，设置mDismissKeyguard为CONTINUE，要求维持解锁状态。否则设置为START，要求执行解锁动作
    ...
    public int finishPostLayoutPolicyLw() {
        ...
        if (mStatusBar != null) {
            if (DEBUG_LAYOUT) Log.i(TAG, "force=" + mForceStatusBar + " forcefkg=" + mForceStatusBarFromKeyguard + " top=" + mTopFullscreenOpaqueWindowState);
