...
public class PhoneStatusBar extends BaseStatusBar {
    ...
    public void start() {
        ...
        // 1。调用父类BaseStatusBar的start方法进行初始化
        ...
        // 2。创建PhoneStatusBarPolicy。PhoneStatusBarPolicy定义了系统通知图标的设置策略
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        try {
            ...
            if (showNav) {
                // 2。从R.layout.navigation_bar中创建导航栏的控件树
        ...
        if (mEmergencyCallLabel != null) {
            ...
            mEmergencyCallLabel.setOnClickListener(new View.OnClickListener() {public void onClick(View v) { }});
...
//        }
    ...
    protected void updateSearchPanel() {
        // 由BaseStatusBAr.updateSearchPanel进行SearchPanel的创建工作。创建好的SearchPanel的控件树会被保存在mSearchPanelView成员中。
    ...
    public void showSearchPanel() {
        ...
        WindowManager.LayoutParams lp = (android.view.WindowManager.LayoutParams) mNavigationBarView.getLayoutParams();
    ...
    private void addNavigationBar() {
        ...
        // 倘若mNavigationBarView为null则表示makeStatusBarView由于WMS.hasNavigationBar的返回值false，即系统不需要导航栏。
        if (mNavigationBarView == null) return;
        // prepareNavigationBarView负责为NavigationBarView中的虚拟按键（KeyButtonView）设置用于响应用户触摸事件的监听器
        // onClickListener或onTouchListener。
        prepareNavigationBarView();
        // 将mWindowManager.addView作为根控件创建导航栏的窗口
...
//            mHandler.post(mStartRevealAnimation);
    ...
    public void setImeWindowStatus(IBinder token, int vis, int backDisposition) {
        ...
        // 通过mCommandQueue.setNavigationIconHints方法修改BACK键的图标
    ...
    private void addStatusBarWindow() {
        // Put up the view
        // 1. 通过getStatusBarHeight方法获取状态栏高度
        ...
        final WindowManager.LayoutParams lp = new WindowManager.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT, // 状态栏的宽度充满整个屏幕宽度
                height, // 高度来自getStatusBarHeight方法
                WindowManager.LayoutParams.TYPE_STATUS_BAR, // 窗口类型
                WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE // 状态栏不接受按键事件
                        // FLAG_TOUCHABLE_WHEN_WAKING这一标记将使得状态栏接受导致设备唤醒的触摸事件
                        // 通常这一事件会在interceptMotionBeforeQueueing的过程中被用于唤醒设备
                        //（或从变暗状态下恢复），而InputDispatcher会阻止这一事件发送给窗口
                        | WindowManager.LayoutParams.FLAG_TOUCHABLE_WHEN_WAKING
                        // FLAG_SPLIT_TOUCH允许状态下支持触摸事件序列的拆分
                        | WindowManager.LayoutParams.FLAG_SPLIT_TOUCH,
        ...
        // 3。创建状态栏的控件树
        makeStatusBarView();
        // 4。通过WindowManager.addView创建状态栏的窗口
