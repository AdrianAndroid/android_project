        ...
        implements WindowManagerService.OnHardKeyboardStatusChangeListener
{
    ...
    HashMap<IBinder,StatusBarNotification> mNotifications = new HashMap<IBinder,StatusBarNotification>();
    ...
    public void setIcon(String slot, String iconPackage, int iconId, int iconLevel, String contentDescription) {
        // 首先同样是权限检查，与registerStatusBar不同，这次要求的是一个系统级别的权限android.permission.STATUS_BAR。
        // 因为设置系统状态区图标的操作不允许普通应用畅叙进行。其他的操作诸如添加一条通知则不需要此权限。
        enforceStatusBar();
    ...
    public void registerStatusBar(IStatusBar bar, StatusBarIconList iconList,
        ...
        // 首先是权限检查。状态栏与导航栏是Android系统的一个十分重要的组件，因此必须避免其他应用调用此方法对状态栏与导航栏进行偷梁换柱。
        // 因此要求方法的调用者必须具有一个签名级的权限
        enforceStatusBarService();

        Slog.i(TAG, "registerStatusBar bar=" + bar);
        // 1。将bar参数保存到mBar成员中。bar的类型是IStatusBar，它就是BaseStatusBar中的CommandQueue的Bp端。
        // 从此之后，StatusBarManagerService将通过mBar与BaseStatusBar进行通信。因此可以理解mBar就是SystemUI中的状态栏与导航栏
        mBar = bar;
        // 2。接下来一次为调用者返回信息副本
        // 系统状态区的图标列表
        synchronized (mIcons) {
    ...
    public IBinder addNotification(StatusBarNotification notification) {
        synchronized (mNotifications) {
            // 1。首先创建一个新的Binder对象用作通知的唯一标识
            IBinder key = new Binder();
            // 2。以key为键，将通知保存在名为mNotification的一个HashMap中
            mNotifications.put(key, notification);
            // 倘若mBar不为null，则表示SystemUI中的BaseStatusBar正处在运行状态。
            // 如7。1。3节所述，mBar就是BaseStatusBar中的CommandQueue的Bp端，负责为StatusBarManagerService
            // 提供访问BaseStatusBar的渠道
            if (mBar != null) {
                try {
                    // 3。通过mBar.addNotification方法将StatusBarNotification实例提交给SystemUI中的BaseStatusBar
