...
public class WindowAnimator {
    ...
    WindowAnimator(final WindowManagerService service) {
        ...
        mAnimationRunnable = new Runnable() {
            ...
            public void run() {
                ...
                // 注意，动画帧处理是一个双锁！首先锁住mWindowMap，然后是mAnimator。因此无论如何
                // 都要避免以相反的顺序使用这个两个帧。例如，当动画系统中有函数访问布局系统时，要尤为重要
                synchronized(mService.mWindowMap) {
                    synchronized(WindowAnimator.this) {
                        // 将布局系统提供的参数"解压"到动画系统，这个话题随后讨论
        ...
        synchronized(layoutToAnim) {
            ...
            if (WindowManagerService.DEBUG_WALLPAPER_LIGHT) {
                if (mWallpaperTarget != layoutToAnim.mWallpaperTarget
                    ...
                    Slog.d(TAG, "Pulling anim wallpaper: target=" + layoutToAnim.mWallpaperTarget + " lower=" + layoutToAnim.mLowerWallpaperTarget + " upper=" + layoutToAnim.mUpperWallpaperTarget);
                }
            }
            ...
            // 从mLayoutToAnim取出与WallPaper及Dimming效果相关的参数
            ...
            for (int i = 0; i < numDisplays; i++) {
                ...
                if (dimParams == null) {
                    ...
                    final WindowStateAnimator existingDimWinAnimator =
                            displayAnimator.mDimParams == null ? null : displayAnimator.mDimParams.mDimWinAnimator;
    ...
    private void updateWindowsLocked(final int displayId) {
        ...
        final WinAnimatorList winAnimatorList = getDisplayContentsAnimatorLocked(displayId).mWinAnimators;
            ...
            if (winAnimator.mSurface != null) {
                ...
                if (wasAnimating && !winAnimator.mAnimating && mWallpaperTarget == win) {
                    ...
                    if (WindowManagerService.DEBUG_LAYOUT_REPEATS) {
                        mService.debugLayoutRepeats("updateWindowsAndWallpaperLocked 2", mPendingLayoutChanges.get(Display.DEFAULT_DISPLAY));
                    }
                }
                ...
                if (mPolicy.doesForceHide(win, win.mAttrs)) {
                    if (!wasAnimating && nowAnimating) {
                        ...
                        if (WindowManagerService.DEBUG_LAYOUT_REPEATS) {
                            mService.debugLayoutRepeats("updateWindowsAndWallpaperLocked 3", mPendingLayoutChanges.get(displayId));
                        }
                    ...
                    if (((mForceHiding == KEYGUARD_ANIMATING_IN)
                        ...
                        if (changed) {
                            if ((mBulkUpdateParams & SET_FORCE_HIDING_CHANGED) != 0 && win.isVisibleNow() /*w.isReadyForDisplay()*/) {
                    ...
                    if (changed && (flags & FLAG_SHOW_WALLPAPER) != 0) {
                        ...
                        setPendingLayoutChanges(Display.DEFAULT_DISPLAY, WindowManagerPolicy.FINISH_LAYOUT_REDO_WALLPAPER);
                        if (WindowManagerService.DEBUG_LAYOUT_REPEATS) {
                            mService.debugLayoutRepeats("updateWindowsAndWallpaperLocked 4", mPendingLayoutChanges.get(Display.DEFAULT_DISPLAY));
                        }
                    }
                }
            }
            // 处理WSA的绘制状态。当WSA的状态READY_TOSHOW时，执行WSA.performShowLocked()将状态切换为HAS_DRAWN
            ...
            if (winAnimator.mDrawState == WindowStateAnimator.READY_TO_SHOW) {
                if (atoken == null || atoken.allDrawn) {
                    if (winAnimator.performShowLocked()) {
                        ...
                        if (WindowManagerService.DEBUG_LAYOUT_REPEATS) {
                            mService.debugLayoutRepeats("updateWindowsAndWallpaperLocked 5", mPendingLayoutChanges.get(displayId));
                        }
                    }
                }
            }
            // 更新AppWindowAnimator的thumbnailLayer。在遍历所有窗口的过程中，寻找一个最靠前的窗口的显示次序作为
            // 其窗口所属的AppWindowToken的缩略图的显示次序
    ...
    // 首先通过updateAppWindowsLocked计算AppWindowToken动画在当前时间所要求的变换
    ...
    // 如果动画渲染过程中，需要布局系统有相应的动作，则将请求放置在mBulkUpdateParams与mPendingLayoutChanges两个成员中，动画渲染完毕后统一通过updateAnimToLayoutLocked()将动作请求发送给布局系统
    private void animateLocked() {
        ...
        // WindowAnmator需要获取当前帧的时间戳
        mCurrentTime = SystemClock.uptimeMillis();
        mBulkUpdateParams = SET_ORIENTATION_CHANGE_COMPLETE;
        // 记录处理当前帧之前的动画状态到wasAnimating中。wasAnimating可以用来确定动画是刚刚开始第一帧或是最后一帧
        // 可以以此进行一些初始化或清理工作
        ...
        try {
            // 1。 首先处理AppWindowAnimatior的动画。这个函数会调用AppWindowAnimator的stepAnimationLocked()
            // 函数以更新AppWindowAnimator的Transformation,这个变换将会在后面影响窗口的Surface的位置
            updateAppWindowsLocked();
            // 接下来会遍历所有的DisplayContentAnimator,并处理其上的旋转动画于窗口动画
            final int numDisplays = mDisplayContentsAnimators.size();
            for (int i = 0; i < numDisplays; i++) {
                final int displayId = mDisplayContentsAnimators.keyAt(i);
                DisplayContentsAnimator displayAnimator = mDisplayContentsAnimators.valueAt(i);
                ...
                if (screenRotationAnimation != null && screenRotationAnimation.isAnimating()) {
                    // 更新旋转动画的Transformation，它一样会在后面影响窗口的Surface位置
                    if (screenRotationAnimation.stepAnimationLocked(mCurrentTime)) {
                        mAnimating = true;
                    } else {
                        // 当旋转完成后，AMS会向Activity发送新的Configuration。这个动作的起点就是这里了。
                        // stepAnimationLocked()的返回值为false时，表示这是动画的最后一帧。
                        // 此时通过向mBulkUpdateParams添加SET_UPDATE_ROTATION棋标，以此通知布局系统旋转动画完毕，需要通知AMS进行更新。
                        mBulkUpdateParams |= SET_UPDATE_ROTATION;
                        screenRotationAnimation.kill();
                        // 由此可见，当屏幕没有处于旋转过程中，DisplayContentsAnimator的mScreenRotationAnimation为null
                        displayAnimator.mScreenRotationAnimation = null;
                    }
                }
                ...
                // 3。 接下来处理WindowStateAnimator的动画。这个处理分为两部分，普通窗口的动画与壁纸动画。
                // 此函数更新了WindowStateAnimator的Transformation的值。同时，在这一步中也会检查窗口绘制状态机的状态，
                // 如果窗口在DisplayContent的布局后处理中完成了commitFinishDrawingLocked()的提交，那么窗口的状态
                // 会在这一步中变为IS_DRAWN，随后prepareSurfaceLocked()会将窗口显示出来
                final WinAnimatorList winAnimatorList = displayAnimator.mWinAnimators;
                final int N = winAnimatorList.size();
                for (int j = 0; j < N; j++) {
                    // 4。 渲染动画
                    // WindowStateAnimator的prepareSurfaceLocked将集合AppWindowAnimator、ScreenRotationAnimator以及
                    // WindowStateAnimator的Transformation到一起，修改Surface的layer、matrix、alpha等属性，从而实现窗口动画的渲染
            ...
            for (int i = 0; i < numDisplays; i++) {
                ...
                // 处理Dimming动画。前面提过，Dimming效果是由一块黑色的Surface完成的Dimming动画的处理分为一下两个步骤
                ...
                // 更新Dimming参数，包括Dimming的尺寸、Dimming的颜色深度以及Dimming的目标等所谓Dimming的目标是指DimmingSurface将要衬于其下的那个窗口
        ...
        // 6。 如果有必要，向布局系统请求一次布局
        // 在动画的过程中，有时需要布局系统"搭把手"，例如前面所见到的，在屏幕旋转动画完成后需要布局系统更新Configuration到AMS。
        // 为了实现从动画系统到布局系统的请求传递，WindowAnimator定义了一下两个成员以在动画渲染的过程中手机所有对布局系统的请求，
        // 并在动画帧渲染结束后，将其通过updateAnimToLayoutLocked一次性发送给布局系统
        if (mBulkUpdateParams != 0 || mPendingLayoutChanges.size() > 0) {
            updateAnimToLayoutLocked();
        }
        // 7 如果仍有动画处于运动状态，通过WMS安排下一帧的处理
        if (mAnimating) {
            synchronized (mService.mLayoutToAnim) {
                // wasAnimating为true而mAnimating为false表示所有动画在这一帧停止
                // 此时需要令布局系统重新进行一次布局。其实，updateAnimTolayoutLocked()就已经向WMS请求一次重新布局了
