...
class WindowStateAnimator {
    ...
    // 是由WMS设置给WSA的一个Animation类的对象，保存了WMS要求窗口执行的动画。当窗口没有运动动画时，mAnimation对象保持为空
    ...
    // 对除Wallpaper与IME之外的窗口来说，其值等于WindowState的mLayer，也就是窗口最终显示次序。
    // 在prepareSurfaceLocked中，它将作为Surface的layer被设置到显示系统
    int mAnimLayer;
    int mLastLayer;
    ...
    // 表示窗口的Surface的透明度。它们两个的区别是，mAlpha是由窗口的LayoutParams.alpha指定的，也就是客户端要求的透明度。
    // 而mShowAlpha是mAlpha在混合了WindowStateAnimator、AppWindowAnimator、ScreenRotateAnimation以及父窗口的WindowStateAnimator的Transofrmation中的
    ...
    // 描述了窗口动画时的"容器尺寸"。容器尺寸一般是DisplayInfo.appWidth/Height，也就是屏幕上排除状态栏/导航栏等系统窗口之后供应用程序显示的区域尺寸。
    // 在为Animation的属性设置带有"p"后缀时，水平方向与垂直方向的参考值就是这两个变量。另外，读者可以回顾一下在布局后的处理过程中使用动画移动窗口位置的情况，
    // 当时将这两个值分别设置成窗口位置变化量，而动画的移动距离都是100%p。
    int mAnimDw;
    int mAnimDh;
    // 本身没有什么意义，它们组合起来就是Surface的变换矩阵。它们是4个Animator的Transformation变换矩阵组合之后（相乘）的缩放与旋转分量
    // 通过Surface.setMatrix方法可以改变Surface最终显示的角度与缩放尺寸。
    ...
    // 保存了窗口的绘制状态。从窗口最初的创建，到最终得以显示到屏幕上，共经历了NO_SURFACE、DRAW_PENDIN、COMMIT_DRAW_PENDING、
    // READ_TO_SHOW和HAS_DRAWN5个状态。后面将深入这5个状态的迁移过程。
    int mDrawState;
    ...
    public void setAnimation(Animation anim) {
        ...
        // 标记以下变量为true， 表示stepAnimation时需要将WSA自身的Transformation参与计划当WSA本身没有动画时
        // 完全可以保持mTranformation为单位变换状态，并在stepAnimation时中不加判断的让其参与计算，但是会降低效率
    ...
    boolean stepAnimationLocked(long currentTime) {
        ...
        if (mService.okToDisplay()) {
            ...
            if (mWin.isDrawnLw() && mAnimation != null) {
                // mAnimation不为null，表示WSA拥有的自己的动画，因此需要计算其Transformation
                mHasTransformation = true;
                mHasLocalTransformation = true;
                // 如果此时mLocalAnimating为false，表示这是WSA的第一帧动画，需要初始化其Animation
                if (!mLocalAnimating) {
                    if (DEBUG_ANIM) Slog.v(TAG, "Starting animation in " + this + " @ " + currentTime + ": ww=" + mWin.mFrame.width() + " wh=" + mWin.mFrame.height() + " dw=" + mAnimDw + " dh=" + mAnimDh + " scale=" + mService.mWindowAnimationScale);
                    // 注意initialzied的参数，动画对象的尺寸为窗口的mFrame的尺寸而容器尺寸为mAnimDw和mAnimDh
                    mAnimation.initialize(mWin.mFrame.width(), mWin.mFrame.height(), mAnimDw, mAnimDh);
                    // mAnimDw && mAnimDh有可能在布局后处理中被改成窗口的移动量，因此最好在其完成mAnimation初始化后将其还原会屏幕上应用程序去的尺寸
                ...
                if ((mAnimation != null) && mLocalAnimating) {
                    // 执行stepAnimation函数更新mTransformation
            ...
            // 如果WSA没有动画或动画已经结束，则标记下述状态变量为false。表示在计算Surface最终的变换时，WSA的mTransformation不参与计算
        ...
        mAnimating = false;
        // 当mLocalAnimating设置为false，这样当下次开始动画时，就可以对Animation进行了初始化了
        mLocalAnimating = false;
        if (mAnimation != null) {
            mAnimation.cancel();
            mAnimation = null;
    ...
    void computeShownFrameLocked() {
        ...
        // 获取父窗口与AppWindowAnimator的Transformation。如果窗口没有父窗口或不属于AppWindowToken，亦或二者不处于动画过程中，则对应的Transformation为null
        ...
        if (mIsWallpaper && mAnimator.mLowerWallpaperTarget == null && mAnimator.mWallpaperTarget != null) {
        ...
        // 获取窗口所在DisplayContent下的ScreenRotationAnimation
        ...
        // 目前，computeShowFrameLocked手机了人中Animator的变换，如果只要有一种Animator正在运行中
        // 就需要整合这些Transformation进行mShowFrame的计算
        if (selfTransformation || attachedTransformation != null
            ...
            // 二位矩阵中，用于存储Transformation矩阵分量的组合结果
            ...
            // 1。 首先将窗口缩放添加到结果矩阵中。postScale函数用于右乘一个缩放矩阵
            tmpMatrix.postScale(mWin.mGlobalScale, mWin.mGlobalScale);
            // 2 将WSA自身的Transformation添加到结果矩阵中。postConcat用于右乘任意矩阵
            if (selfTransformation) {
                tmpMatrix.postConcat(mTransformation.getMatrix());
            }
            // 3。将窗口的实际位置添加到结果矩阵中。postTranslate函数用于右乘一个平移矩阵
            tmpMatrix.postTranslate(frame.left + mWin.mXOffset, frame.top + mWin.mYOffset);
            // 4 将父窗口的动画变换添加到结果矩阵中
            if (attachedTransformation != null) {
                tmpMatrix.postConcat(attachedTransformation.getMatrix());
            }
            // 5 将AppWindowAnimator的变换添加到结果矩阵中
            if (appTransformation != null) {
                tmpMatrix.postConcat(appTransformation.getMatrix());
            }
            // 6 将UniverseBackground的变换添加到结果矩阵中。
            if (mAnimator.mUniverseBackground != null) {
                tmpMatrix.postConcat(mAnimator.mUniverseBackground.mUniverseTransform.getMatrix());
            }
            // 7 将ScreenRotationAnmation的变换加入到结果矩阵中
            if (screenAnimation) {
            ...
            // 结果矩阵已经计算完成，提取它们的缩放旋转子阵作为Surface的变换矩阵，并将平移分量保存到mShowFrame中
            mHaveMatrix = true;
            ...
            // 计算mShowFrame，注意只有left和top分量取自变换结果，width和height捅mFrame
            ...
            if (!mService.mLimitedAlphaCompositing
                    ...
                    || (mWin.isIdentityMatrix(mDsDx, mDtDx, mDsDy, mDtDy) && x == frame.left && y == frame.top))) {
            ...
            return;
        } else if (mIsWallpaper && (mAnimator.mPendingActions & WindowAnimator.WALLPAPER_ACTION_PENDING) != 0) {
            return;
        }
    ...
    public void prepareSurfaceLocked(final boolean recoveringMemory) {
        ...
        // 还是setSurfaceBoundariesLocked，将计算好的mShowFrame设置为Surface的位置，完成平移操作。
        // 注意，尺寸仍为mFrame的尺寸。动画所要求的缩放变换将通过变换矩阵完成
    ...
    boolean applyAnimationLocked(int transit, boolean isEntrance) {
        ...
        // is running.
        if (mService.okToDisplay()) {
            // 首先尝试由WindowManagerPolicy获取合适的动画资源。
            // WindowManagerPolicy的selectAnimationLw为状态栏、导航栏等系统窗口分配了一些特殊的动画
            ...
            if (anim != 0) {
                ...
                if (attr >= 0) {
                    // 保存选取的Animatin对象
                    a = mService.loadAnimation(UserHandle.getUserId(mWin.mOwnerUid), mWin.mAttrs, attr);
                }
            }
