...
public final class ActivityThread {
    ...
    private class ApplicationThread extends ApplicationThreadNative {
        ...
        public final void scheduleStopActivity(IBinder token, boolean showWindow,
                                               int configChanges) {
            queueOrSendMessage( //�ú����ڲ�����һ��handler���Ͷ�Ӧ����Ϣ
    ...
    public void installSystemApplicationInfo(ApplicationInfo info) {
        synchronized (this) {
            // ���ص�ContextImpl����֮ǰ��ams��main����һ���д������Ǹ�����
            ContextImpl context = getSystemContext();
            //�ֵ���init��ʼ����context���ǲ����ظ�����init��
    ...
    final void handleResumeActivity(IBinder token, boolean clearHide, boolean isForward, boolean reallyResume) {
        ...
        // we are back active so skip it.
        unscheduleGcIdler();
        // 1。在performResumeActivity中，Activity.onResume方法将会被调用
        ...
        if (r != null) {
            ...
            // 2。创建窗口
            // 当ActivityClientRecord的window成员为null时，表示此Activity尚未创建窗口。此时需要将PhoneWindow中的控件树交给WindowManager完成窗口的创建。
            // 这种情况对应于Activity初次创建的情况（即onCreate被调用的情况）。如果Activity因为某种原因被暂停，如新的Activity覆盖其上或者用户按下了HOME键，
            // 虽说Activity不再与Resume状态，但是其窗口并没有从WMS中移除，只不过它不可见而已。
            if (r.window == null && !a.mFinished && willBeVisible) {
                // 获取PhoneWindowManager的实例
                r.window = r.activity.getWindow();
                // 后去DecorView
                View decor = r.window.getDecorView();
                // 注意Activity的窗口在初创时是不可见的。因为尚不确定是否真的要显示窗口给用户
            ...
            if (!r.activity.mFinished && willBeVisible && r.activity.mDecor != null && !r.hideForNow) {
                ...
                if (r.activity.mVisibleFromClient) {
                    // 最后通过Activity.makeVisible使得Activity可见
    ...
    private void attach(boolean system) {
        ...
        } else { //ϵͳ���̵Ĵ������̣������ֻ��systemsserver�д���
            // Don't set application object here -- if the system crashes,
            // we can't display an alert, we just want to die die die.��
            // ����ddmsʱ������systemserver������Ϊsystemprocess
            android.ddm.DdmHandleAppName.setAppName("system_process",
                    UserHandle.myUserId());
            try {
                // activitythread�ļ�Ա�󽫳����������ĵķ���
                mInstrumentation = new Instrumentation();
                ContextImpl context = new ContextImpl();
                // ��ʼ��context��ע���һ������ֵΪgetsytemcontext
                context.init(getSystemContext().mPackageInfo, null, this);
                // ����Instrumentation����һ��application����
                Application app = Instrumentation.newApplication(Application.class, context);
                mAllApplications.add(app); //application����
                mInitialApplication = app;
                app.onCreate();
            } catch (Exception e) {
                throw new RuntimeException(
    ...
    // Instrumentation: Instrumentation��һ�������ࡣ���������ʱ��ϵͳ�ȴ���������ͨ����������������������⣬ϵͳ�����֮���
    // 		����Ҳ��ͨ��Instrumentation�����ݣ�����Instrumentation���ܼ�����Щ����Ľ�������ˡ���ʵ��ʹ���У����ǿ��Դ���Instrumentation
    //		����������������Ӧ�Ĵ�������߿ɲ�ѯandroid�е�junit��ʹ�����˽�Instrumentation�����á�
    // Application : ������һ��ȫ��application״̬��application��androidmanifest��xml�е�application��ǩ��������ʵ��ʹ��ʱ�趨��application��������
    //
    // Context context��һ���ӿڣ�ͨ�������Ի�ȡ������application��Ӧ����Դ���࣬���ð�����application�е��Ĵ������
    //

    public static ActivityThread systemMain() {
        HardwareRenderer.disable(true); //��ֹӲ����Ⱦ����
        // ����һ��activitythread�����乹�캯���ǳ���
        ActivityThread thread = new ActivityThread();
        thread.attach(true); //��������attach������ע�⴫�ݵĲ���Ϊtrue
        return thread;
    }
