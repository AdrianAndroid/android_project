...
final class WindowState implements WindowManagerPolicy.WindowState {
    ...
    final Rect mFrame = new Rect(); // 描述了窗口的位置于尺寸
    ...
    final Rect mParentFrame = new Rect(); // 保存了pf参数
    ...
    WindowState(WindowManagerService service, Session s, IWindow c, WindowToken token,
        ...
        // * 子序越大，则子窗口相对于其兄弟窗口的位置越靠前。对父窗口而言，其主序取决于其类型，其子序保持为0。
        // 而子窗口的主序与其父窗口一样，子序则取决于其类型。从上述代码可以看到，主序与子序的分配工作是由WindowManagerPolicy
        // 的两个成员函数windowTypeToLayerLw()和subWindowTypeToLayerLw()完成的
        if ((mAttrs.type >= FIRST_SUB_WINDOW && mAttrs.type <= LAST_SUB_WINDOW)) {
            // The multiplier here is to reserve space for multiple
            // windows in the same type layer.
            // 这里的mPolicy就是WindowManagerPolicy
    ...
    /**
     *
     * @param pf ParentFrame
     * @param df DisplayFrame
     * @param cf ContentFrame
     * @param vf VisiableFrame
     */
    @Override
    public void computeFrameLw(Rect pf, Rect df, Rect cf, Rect vf) {
        ...
        visible.set(vf);
        // WindowState的mFrame成员变量表示窗口的当前位置于尺寸
        ...
        // 调用Gravity.apply函数根据gravity、pf、w、h、x、y计算窗口的mFrame。也就是说对pf对窗口的位置起到决定性作用
        ...
        // 调用Gravity.applyDisplay函数根据gravity,df来修正mFrame。其原则为，mFrame必须完全位于df之内。经过修正后的mFrame就是窗口的最终位置和尺寸
        Gravity.applyDisplay(mAttrs.gravity, df, frame);

        // Make sure the system, content and visible frames are inside of the
        // final window frame.
        // 根据计算出的mFrame调整mContentFrame和mVisiableFrame。窗口最终的mVisibleFrame和
        // mContentFrame是系统当前的的cf或vf于窗口的mFrame的交集
        ...
        // visibleInsets。用来指示mVisibaleFrame到mFrame的4条边界的距离
    ...
    public final boolean canReceiveKeys() {
        return     isVisibleOrAdding() // 窗口不会即将被移除
                && (mViewVisibility == View.VISIBLE) // 窗口可见
                // LayoutParams没有指定FLAG_NOT_FOCUSABLE选项
    ...
    String makeInputChannelName() {
        return Integer.toHexString(System.identityHashCode(this)) + " " + mAttrs.getTitle();
    }
